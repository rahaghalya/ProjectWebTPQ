...
if ($filter_jabatan) { $where_cond[] = "jabatan LIKE ?"; $params[] = "%$filter_jabatan%"; $types .= "s"; }
if ($filter_keterangan) { $where_cond[] = "keterangan LIKE ?"; $params[] = "%$filter_keterangan%"; $types .= "s"; }
    ...
    <div class="container">
      ...
      <?php switch ($act): 
        ...
        <?php case 'edit': 
          ...
          <?php } else { echo "<div class='alert alert-danger'>Data tidak ditemukan</div>"; } ?>
        <?php break; ?>
          ...
          <div class="table-responsive">
            <table class="table table-striped table-hover">
              <thead class="table-dark">
                <tr>
                  <th><a href="?sort=id_guru&order=<?= ($sort_order=='ASC'?'DESC':'ASC') ?>" class="text-white">ID</a></th>
                  <th><a href="?sort=nama&order=<?= ($sort_order=='ASC'?'DESC':'ASC') ?>" class="text-white">Nama Guru</a></th>
                  <th><a href="?sort=jabatan&order=<?= ($sort_order=='ASC'?'DESC':'ASC') ?>" class="text-white">Jabatan</a></th>
                  <th><a href="?sort=keterangan&order=<?= ($sort_order=='ASC'?'DESC':'ASC') ?>" class="text-white">Status</a></th>
                  <th>Aksi</th>
                </tr>
              </thead>
              <tbody>
                <?php if ($result_tabel && $result_tabel->num_rows > 0): ?>
                  ...
                  <?php endwhile; ?>
                <?php else: ?>
                  <tr><td colspan="5" class="text-center">Data Guru Kosong</td></tr>
                <?php endif; ?>
              </tbody>
            </table>
          </div>
