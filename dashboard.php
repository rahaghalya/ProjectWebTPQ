...
// 1. Ambil Total Santri (Hanya yang statusnya 'Aktif')
$query_santri = "SELECT COUNT(*) AS total FROM santri WHERE keterangan = 'Aktif'";
$result_santri = mysqli_query($koneksi, $query_santri);
$total_santri = mysqli_fetch_assoc($result_santri)['total'] ?? 0;

// 2. Ambil Total Guru
$query_guru = "SELECT COUNT(*) AS total FROM guru"; // Asumsi nama tabel 'guru'
$result_guru = mysqli_query($koneksi, $query_guru);
$total_guru = mysqli_fetch_assoc($result_guru)['total'] ?? 0;

// Ambil Total Alumni (yang sudah tidak aktif)
$query_alumni = "SELECT COUNT(*) AS total FROM santri WHERE keterangan IN ('Keluar', 'Lulus', 'Pindah')";
$result_alumni = mysqli_query($koneksi, $query_alumni);
$total_alumni = mysqli_fetch_assoc($result_alumni)['total'] ?? 0;
            ...
            <div class="row">
                ...
                <div class="col-6 col-md-6 col-lg-3 mb-4">
                    <div class="stat-card">
                        ...
                        <h5>Total Alumni</h5>
                        <p class="text-muted small">Santri Lulus/Pindah</p>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const stats = {
        santri: <?php echo $total_santri; ?>,
        guru: <?php echo $total_guru; ?>,
        alumni: <?php echo $total_alumni; ?>,
   ...
   const totalSantriEl = document.getElementById('total-santri');
    ...
    animateCounter(totalSantriEl, stats.santri);
    animateCounter(totalGuruEl, stats.guru);
    animateCounter(totalAlumniEl, stats.alumni);
